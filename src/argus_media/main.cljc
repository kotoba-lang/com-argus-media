(ns argus_media.main
  "Kotodama WASM entrypoint for the Argus_media clean-room actor (L4) — Clojure port.
  ...
  facts (`argus_media.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "argus_media")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"argus_media.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "argus_media-compat" :tier tier :entities entities} 200])
...
(defn start! [] :argus_media-compat/ready)
